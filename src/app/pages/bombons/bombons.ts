...
@Component({
  selector: 'app-bombons',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './bombons.html',
  styleUrls: ['./bombons.css']
})
export class Bombons implements OnInit {
  produtos: Produto[] = [];
  totalItens = 0;
  ...
  ngOnInit(): void {
    this.produtosService.produtos$.subscribe(produtos => {
      this.produtos = produtos.filter(p => p.categoria === 'bombons' && p.ativo);
    });
